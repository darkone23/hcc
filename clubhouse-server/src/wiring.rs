...
impl ServerWiring {
    pub async fn database(config: &ServerConfig) -> Result<DatabaseConnection, DbErr> {
        let db = Database::connect(&config.sql_connection_url).await;
        db
    }
    ...
    pub fn init_server_config() -> ServerConfig {
        ...
        ServerConfig {
            ...
            sql_connection_url: env::var("HCC_SQL_CONNECTION_URL")
                .expect("Invalid configuration: HCC_SQL_CONNECTION_URL required"),
